...
var require_server_destroy = __commonJS({
  "node_modules/server-destroy/index.js"(exports, module2) {
...
function extractContentInOrder(container) {
  ...
  const topLevelElements = Array.from(allElements).filter((element) => {
    return !Array.from(allElements).some(
      (otherElement) => otherElement !== element && element instanceof Element && otherElement instanceof Element && otherElement.contains(element) && // Only exclude if the parent is a block-level element that processes its own content
      (otherElement.tagName.toLowerCase() === "blockquote" || otherElement.tagName.toLowerCase() === "ul" || otherElement.tagName.toLowerCase() === "ol" || otherElement.classList.contains("footnote") || otherElement.tagName.toLowerCase() === "figure" || otherElement.classList.contains("captioned-image-container"))
    );
  });
...
`;
      ...
      case "div":
        if (element instanceof Element && element.classList.contains("footnote")) {
          const footnoteNum = (_a = element.querySelector(".footnote-number")) == null ? void 0 : _a.textContent;
          const footnoteContent = (_b = element.querySelector(".footnote-content")) == null ? void 0 : _b.textContent;
...
function processImage(img) {
  ...
  if (!src || src.includes("tracking") || img.getAttribute("width") === "1" || img.getAttribute("height") === "1") {
    return "";
  }
...
function processCaptionedImageContainer(container) {
  ...
  if (!imageMarkdown)
    return "";
  const caption = container.querySelector(".image-caption, .caption, figcaption, .subtitle") || ((_a = container.nextElementSibling) == null ? void 0 : _a.classList.contains("caption")) ? container.nextElementSibling : null;
  if (caption) {
    const captionText = caption.textContent.trim();
    return `${imageMarkdown}
*${captionText}*`;
...
function handleEmphasisFormatting(content, marker) {
      ...
      switch (tagName) {
        ...
        case "a":
          ...
          if (href && !href.includes("unsubscribe")) {
            const isInternalAppAction = href.includes("substack.com/app-link") && !href.includes("redirect=app-store") && (href.includes("action=like") || href.includes("action=share") || href.includes("action=comment") || href.includes("submitLike=true"));
...
function cleanMarkdown(markdown) {
  return markdown.replace(/\n{3,}/g, "\n\n").replace(/\s+([,.!?;:])/g, "$1").replace(/[ \t]+$/gm, "").replace(/^>\s*$/gm, ">").replace(/^(\s*[-*+]\s*)\s+/gm, "$1").trim();
}
...
var NoteModal = class extends import_obsidian.Modal {
  constructor(app, plugin, selectedText, sourceFile, extractFolder) {
    super(app);
    this.extractTitle = "";
  ...
  onOpen() {
    ...
    contextTextArea.addEventListener("input", (e) => {
      this.additionalContext = e.target.value;
    });
    new import_obsidian.Setting(contentEl).setName("Tags").setDesc("Add tags (comma-separated)").addText((text) => text.setPlaceholder("tag1, tag2, extract, important").setValue(this.tags).onChange((value) => {
      this.tags = value;
    }));
  ...
  async saveExtract() {
    if (!this.extractTitle.trim()) {
      new import_obsidian.Notice("Please enter a title for the extract");
      return;
    }
    try {
      ...
      if (existingFile) {
        new import_obsidian.Notice("A file with this title already exists");
        return;
      }
      ...
      new import_obsidian.Notice(`Extract saved to ${filePath}`);
      this.close();
    } catch (error) {
      console.error("Failed to save extract:", error);
      new import_obsidian.Notice("Failed to save extract");
    }
  }
  createExtractMarkdown() {
    const now2 = new Date();
    const dateStr = now2.toISOString().split("T")[0];
    const timeStr = now2.toLocaleTimeString();
    const tagArray = this.tags.split(",").map((tag) => tag.trim()).filter((tag) => tag.length > 0);
...
var AbstractSimplePool = class {
  ...
  subscribe(relays, filter, params) {
    ...
    for (let i2 = 0; i2 < relays.length; i2++) {
      const url2 = normalizeURL(relays[i2]);
      if (!request.find((r) => r.url === url2)) {
        request.push({ url: url2, filter });
      }
    }
  ...
  subscribeMap(requests, params) {
    ...
    for (const req of requests) {
      const { url: url2, filter } = req;
      if (!grouped.has(url2))
        grouped.set(url2, []);
      grouped.get(url2).push(filter);
    }
    const groupedRequests = Array.from(grouped.entries()).map(([url2, filters]) => ({ url: url2, filters }));
    if (this.trackRelays) {
    ...
    const allOpened = Promise.all(
      groupedRequests.map(async ({ url: url2, filters }, i2) => {
        let relay;
        try {
...
async function searchDomain(domain, query = "") {
  try {
    const url2 = `https://${domain}/.well-known/nostr.json?name=${query}`;
...
async function queryProfile(fullname) {
  ...
  try {
    const url2 = `https://${domain}/.well-known/nostr.json?name=${name}`;
    const res = await _fetch(url2, { redirect: "manual" });
    if (res.status !== 200) {
      throw Error("Wrong response code");
    }
...
function finishRepostEvent(t, reposted, relayUrl, privateKey) {
  var _a, _b;
  let kind;
  const tags = [...(_a = t.tags) != null ? _a : [], ["e", reposted.id, relayUrl], ["p", reposted.pubkey]];
  ...
  while (index < max) {
    ...
    if (content.substring(u - 5, u) === "nostr") {
      ...
      try {
        ...
        if (/\.(png|jpe?g|gif|webp)$/i.test(url2.pathname)) {
          yield { type: "image", url: url2.toString() };
          index = end;
          prevIndex = index;
          continue;
        }
        if (/\.(mp4|avi|webm|mkv)$/i.test(url2.pathname)) {
          yield { type: "video", url: url2.toString() };
          index = end;
          prevIndex = index;
          continue;
        }
        if (/\.(mp3|aac|ogg|opus)$/i.test(url2.pathname)) {
...
var NostrNIP23Client = class {
  ...
  async getNostrData(kind, authors, limit2, processNote, since) {
    try {
      ...
      if (since) {
        filter.since = Math.floor(since.getTime() / 1e3);
      }
      const events = [];
      ...
      if (kind == 0) {
        ...
        return {
          ...
          lastUpdated: new Date(latestEvent.created_at * 1e3)
        };
      } else if (kind == 30023) {
  ...
  npubToHex(pubkey) {
    try {
      const decoded = nip19_exports.decode(pubkey);
...
if (!import_obsidian2.Platform.isMobile) {
  try {
    http = require("http");
    url = require("url");
    destroyer = require_server_destroy();
...
var MobileOAuthWaitingModal = class extends import_obsidian2.Modal {
  constructor(app, onCancel, onManualCode) {
    super(app);
    ...
    if (!this.showManualEntry) {
      ...
      submitButton.onclick = () => {
        ...
        if (code) {
          ...
          new import_obsidian2.Notice("Please enter the authorization code");
        }
      };
...
var CatchementPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.syncInterval = null;
    this.nostrSyncInterval = null;
  ...
  async onload() {
    ...
    this.addRibbonIcon("globe", "Sync Articles", (evt) => {
      this.syncAllContent();
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      name: "TestReauth",
      callback: async () => {
        this.settings.expiry_date = Date.now() - 1e3;
        this.settings.refresh_token_expiry = Date.now() - 1e3;
        await this.saveSettings();
        new import_obsidian2.Notice("Tokens expired, triggering reauthorization...");
        setTimeout(() => {
          this.syncNewsletters();
        }, 500);
      }
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (selection && selection.trim().length > 0) {
          menu.addItem((item) => {
            item.setTitle("Take Note").setIcon("quote-glyph").onClick(() => {
  ...
  async handleOAuthCallback(params) {
    ...
    if (params.error) {
      ...
      new import_obsidian2.Notice(`Authentication failed: ${errorMsg}`);
    ...
    if (params.state !== this.settings.pkce_state) {
      ...
      new import_obsidian2.Notice("Authentication failed: Security validation failed");
      if (this.mobileAuthRejecter) {
    ...
    if (!params.code) {
      ...
      new import_obsidian2.Notice("Authentication failed: No authorization code received");
      if (this.mobileAuthRejecter) {
    ...
    try {
      ...
      await this.saveSettings();
      new import_obsidian2.Notice("Successfully authenticated with Gmail!");
  ...
  createNote(editor, view) {
    const selectedText = editor.getSelection();
    if (!selectedText || selectedText.trim().length === 0) {
      new import_obsidian2.Notice("Please select some text to extract");
      return;
    }
    if (!view.file) {
  ...
  onunload() {
    ...
    if (server_ && !import_obsidian2.Platform.isMobile) {
      server_.close();
    }
  }
  async loadSettings() {
  ...
  async initializeNostr() {
    ...
    try {
      ...
      new import_obsidian2.Notice("Failed to connect to Nostr relays");
    }
  }
  startNostrAutoSync() {
    if (this.nostrSyncInterval) {
    ...
    if (this.settings.nostrSyncFrequency > 0 && this.settings.nostrEnabled) {
      this.nostrSyncInterval = window.setInterval(() => {
        this.syncNostrArticles();
      }, this.settings.nostrSyncFrequency * 60 * 1e3);
    }
  }
  async formatFilename(name) {
  ...
  base64URLEncode(buffer) {
    const bytes4 = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
    let binary = "";
    bytes4.forEach((byte) => binary += String.fromCharCode(byte));
    return btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
  }
  generatePKCEVerifier() {
    const randomBytes3 = this.generateRandomBytes(64);
    return this.base64URLEncode(randomBytes3);
  }
  async generatePKCEChallenge(verifier) {
    const encoder = new TextEncoder();
    const data = encoder.encode(verifier);
    const webCrypto = this.getWebCrypto();
    const hash3 = await webCrypto.subtle.digest("SHA-256", data);
    return this.base64URLEncode(hash3);
  }
  generateState() {
    const randomBytes3 = this.generateRandomBytes(32);
    return this.base64URLEncode(randomBytes3);
  }
  async syncNostrArticles() {
    if (!this.settings.nostrEnabled) {
      new import_obsidian2.Notice("Nostr sync is disabled in settings");
      return;
    }
    if (!this.nostrClient) {
    ...
    if (!this.nostrClient) {
      new import_obsidian2.Notice("Failed to initialize Nostr client");
      return;
    }
    if (this.settings.nostrFollowedAuthors.length === 0) {
      new import_obsidian2.Notice("No Nostr authors configured. Please add pubkeys in settings.");
      return;
    }
    new import_obsidian2.Notice("Syncing Nostr long-form articles...");
    try {
      ...
      const sinceDate = this.settings.nostrLastSyncTime ? new Date(this.settings.nostrLastSyncTime * 1e3) : void 0;
      ...
      this.nostrClient.getNostrData(30023, this.settings.nostrFollowedAuthors, queryLimit, (article) => {
        this.processNostrArticle(article, existingFiles).then((processed) => {
          if (processed) {
            processedCount++;
          }
        }).catch((error) => {
      ...
      this.settings.nostrLastSyncTime = Math.floor(Date.now() / 1e3);
      await this.saveSettings();
      setTimeout(() => {
        new import_obsidian2.Notice(`Processed ${processedCount} new Nostr articles`);
      }, 7e3);
    } catch (error) {
      console.error("Nostr sync failed:", error);
      new import_obsidian2.Notice("Failed to sync Nostr articles");
    }
  }
  async processNostrArticle(article, existingFiles) {
    try {
  ...
  createNostrMarkdownContent(article) {
    const title = article.parsed.title || "Untitled Article";
    const author = this.settings.nostrFollowedAuthors.find((author2) => author2.pubkey === article.pubkey);
    const publishedDate = article.parsed.published_at ? new Date(article.parsed.published_at * 1e3).toISOString() : new Date(article.created_at * 1e3).toISOString();
...
`;
  ...
  async syncAllContent() {
    new import_obsidian2.Notice("Syncing all content sources...");
    const promises = [];
    ...
    try {
      ...
      new import_obsidian2.Notice("All content sources synced successfully");
    } catch (error) {
      console.error("Failed to sync all content:", error);
      new import_obsidian2.Notice("Some content sources failed to sync");
    }
  }
  // ============================================
  ...
  async _performMobileAuthorization() {
    const verifier = this.generatePKCEVerifier();
    const challenge2 = await this.generatePKCEChallenge(verifier);
  ...
  async _performDesktopAuthorization() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const authTimeout = setTimeout(() => {
        if (!completed) {
          ...
          if (server_ == null ? void 0 : server_.listening) {
            server_.close();
          }
  ...
  async exchangeCodeForTokens(code, verifier) {
    ...
    return {
      ...
      expiry_date: Date.now() + (tokenData.expires_in || 3600) * 1e3
    };
  }
  getPortFromURI(uri) {
    ...
    if (!this.settings.access_token || !this.settings.refresh_token || !notExpired) {
      try {
        ...
        if (error.message.includes("timeout")) {
          ...
          new import_obsidian2.Notice("Authorization timed out. Please try syncing again.");
          return false;
        }
        ...
        new import_obsidian2.Notice("Failed to authenticate with Google");
        return false;
      }
    }
    return true;
  ...
  startGmailAutoSync() {
    ...
    if (this.settings.syncFrequency > 0) {
      this.syncInterval = window.setInterval(() => {
        this.syncNewsletters();
      }, this.settings.syncFrequency * 60 * 1e3);
    }
  }
  async refreshAccessToken() {
    console.log(`Refreshing Token...`);
    if (!this.settings.refresh_token) {
      new import_obsidian2.Notice("No refresh token available. Please re-authenticate.");
      return false;
    }
    try {
      const params = new URLSearchParams({
      ...
      if (!response.ok) {
        ...
        if (errorData.error === "invalid_grant") {
          ...
          await this.saveSettings();
          new import_obsidian2.Notice("Session expired. Please re-authenticate.");
        }
        return false;
      }
      const tokenData = await response.json();
      this.settings.access_token = tokenData.access_token;
      this.settings.expiry_date = Date.now() + (tokenData.expires_in || 3600) * 1e3;
      await this.saveSettings();
  ...
  async listGmailMessages(params = {}) {
    ...
    try {
      ...
      new import_obsidian2.Notice("Failed to fetch emails from Gmail");
      return null;
    }
  }
  async _fetchGmailMessages(params = {}) {
  ...
  async _fetchGmailMessage(messageId) {
    const response = await fetch(
  ...
  async syncNewsletters() {
    new import_obsidian2.Notice("Syncing Substack newsletters...");
    try {
      ...
      if (!messagesResponse || !messagesResponse.messages) {
        new import_obsidian2.Notice("No Substack newsletters found");
      ...
      new import_obsidian2.Notice(`Synced ${processedCount} new Substack newsletters`);
    } catch (error) {
      console.error("Sync failed:", error);
      new import_obsidian2.Notice("Failed to sync newsletters");
    }
  }
  async processMessage(message, existingFiles) {
    var _a, _b, _c, _d;
    const headers = message.payload.headers;
    const subject = ((_a = headers.find((h) => h.name === "Subject")) == null ? void 0 : _a.value) || "Untitled";
    const from = ((_b = headers.find((h) => h.name === "From")) == null ? void 0 : _b.value) || "Unknown";
    const date = (_c = headers.find((h) => h.name === "Date")) == null ? void 0 : _c.value;
    const replyto = (_d = headers.find((h) => h.name == "Reply-To")) == null ? void 0 : _d.value;
    if (!replyto) {
      return false;
    }
  ...
  extractEmailContent(message) {
    ...
    if (message.payload.parts) {
      for (const part of message.payload.parts) {
        if (part.mimeType === "text/html" && ((_a = part.body) == null ? void 0 : _a.data)) {
          content = this.decodeBase64(part.body.data);
          break;
        } else if (part.mimeType === "text/plain" && ((_b = part.body) == null ? void 0 : _b.data)) {
          content = this.decodeBase64(part.body.data);
        }
      }
    } else if ((_c = message.payload.body) == null ? void 0 : _c.data) {
  ...
  decodeBase64(data) {
    try {
      const base642 = data.replace(/-/g, "+").replace(/_/g, "/");
      return atob(base642);
    } catch (error) {
  ...
  createMarkdownContent(subject, publication, from, date, content) {
    const frontmatter = `---
title: "${subject}"
...
var CatchmentSettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.nostrClient = new NostrNIP23Client([
      "wss://relay.damus.io",
      "wss://nos.lol",
      "wss://relay.snort.social"
    ]);
  ...
  display() {
    ...
    if (import_obsidian2.Platform.isMobile) {
      containerEl.createEl("div", {
        text: "\u{1F4F1} Running on mobile - using Obsidian URI scheme for seamless authentication",
    ...
    new import_obsidian2.Setting(containerEl).setName("Articles Folder").setDesc("Folder where substack and nostr articles will be saved").addText((text) => text.setPlaceholder("Substack Newsletters").setValue(this.plugin.settings.catchementFolder).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Authenticate with Gmail").setDesc("Connect your Gmail account to sync Substack newsletters").addButton((button) => button.setButtonText(this.plugin.settings.access_token ? "Re-authenticate" : "Connect Gmail").setCta().onClick(async () => {
      try {
        await this.plugin.initializeGoogleAuth();
        this.display();
      } catch (error) {
        console.error("Authentication failed:", error);
        new import_obsidian2.Notice("Authentication failed. Please try again.");
      }
    }));
    containerEl.createEl("h3", { text: "Sync Configuration" });
    new import_obsidian2.Setting(containerEl).setName("Max Emails").setDesc("Maximum number of emails to fetch per sync").addSlider((slider) => slider.setLimits(10, 200, 10).setValue(this.plugin.settings.maxEmails).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.maxEmails = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Gmail Auto-sync Frequency").setDesc("How often to automatically sync Gmail (in minutes, 0 to disable)").addSlider((slider) => slider.setLimits(0, 1440, 0).setValue(this.plugin.settings.syncFrequency).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.syncFrequency = value;
      await this.plugin.saveSettings();
      this.plugin.startGmailAutoSync();
    }));
    new import_obsidian2.Setting(containerEl).setName("Substack Articles Icon Name").setDesc("Name of Icon to display beside substack articles.").addText((text) => text.setPlaceholder("Icon Name").setValue(this.plugin.settings.substackIcon).onChange(async (value) => {
      this.plugin.settings.substackIcon = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Enable Nostr Sync").setDesc("Enable syncing of long-form articles from Nostr").addToggle((toggle) => toggle.setValue(this.plugin.settings.nostrEnabled).onChange(async (value) => {
      this.plugin.settings.nostrEnabled = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.nostrEnabled) {
      new import_obsidian2.Setting(containerEl).setName("Nostr Auto-sync Frequency").setDesc("How often to automatically sync Nostr articles (in minutes, 0 to disable)").addSlider((slider) => slider.setLimits(0, 1440, 30).setValue(this.plugin.settings.nostrSyncFrequency).setDynamicTooltip().onChange(async (value) => {
        this.plugin.settings.nostrSyncFrequency = value;
        await this.plugin.saveSettings();
        this.plugin.startNostrAutoSync();
      }));
      new import_obsidian2.Setting(containerEl).setName("Nostr Articles Icon Name").setDesc("Name of Icon to display beside nostr articles.").addText((text) => text.setPlaceholder("Icon Name").setValue(this.plugin.settings.nostrIcon).onChange(async (value) => {
      ...
      new import_obsidian2.Setting(containerEl).setName("Nostr Relays").setDesc("List of Nostr relays to connect to").addTextArea((text) => {
        text.inputEl.rows = 6;
        text.inputEl.cols = 50;
        return text.setPlaceholder("wss://relay.damus.io\nwss://nos.lol\nwss://relay.snort.social").setValue(this.plugin.settings.nostrRelays.join("\n")).onChange(async (value) => {
          this.plugin.settings.nostrRelays = value.split("\n").map((relay) => relay.trim()).filter((relay) => relay.length > 0);
          await this.plugin.saveSettings();
        });
      });
      containerEl.createEl("div", {
        text: "\u{1F4A1} Tip: You can paste npub1... format or hex pubkeys. The plugin will handle both formats.",
        cls: "setting-item-description",
      ...
      addButton.onclick = async () => {
        ...
        if (npub && !nostrFollowedPubkeys.includes(npub)) {
          ...
          quickAddInput.value = "";
          this.display();
          new import_obsidian2.Notice("Author added successfully!");
        } else if (!npub) {
          new import_obsidian2.Notice("Please enter a valid npub");
        } else {
          new import_obsidian2.Notice("Author already in the list");
        }
      };
      if (this.plugin.settings.nostrFollowedAuthors.length > 0) {
        ...
        this.plugin.settings.nostrFollowedAuthors.forEach((author, index) => {
          ...
          removeButton.onclick = async () => {
            ...
            await this.plugin.saveSettings();
            this.display();
            new import_obsidian2.Notice("Author removed");
          };
        });
      }
    }
    containerEl.createEl("h3", { text: "Manual Sync" });
    new import_obsidian2.Setting(containerEl).setName("Sync Gmail Newsletters").setDesc("Manually sync Substack newsletters from Gmail").addButton((button) => button.setButtonText("Sync Gmail").setCta().onClick(() => {
      this.plugin.syncNewsletters();
    }));
    if (this.plugin.settings.nostrEnabled) {
      new import_obsidian2.Setting(containerEl).setName("Sync Nostr Articles").setDesc("Manually sync long-form articles from Nostr").addButton((button) => button.setButtonText("Sync Nostr").setCta().onClick(() => {
        this.plugin.syncNostrArticles();
      }));
    }
    new import_obsidian2.Setting(containerEl).setName("Sync All Sources").setDesc("Sync content from all enabled sources").addButton((button) => button.setButtonText("Sync All").setClass("mod-warning").onClick(() => {
